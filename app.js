...
app.post(`${parsefontRoot}/convert`, upload.single('file'), (req, res) => {
  parsefont.get({ svgFile: path.join(__dirname, `./${req.file.path}`), ...req.body })
  .then(fiels => {
    var zip = new JSZip();
    zip.file(`index.html`, fiels.html);
    zip.file(`style.css`, fiels.style);
    zip.file(`fontsvg.svg`, fiels.fontSvg);
    zip.file(`font.ttf`, fiels.svg2ttfbuf);
    zip.file(`font.eot`, fiels.svg2eotbuf);
    zip.file(`font.woff`, fiels.svg2woffbuf);
